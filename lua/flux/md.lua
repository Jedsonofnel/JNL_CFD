...
local function is_bq(s)
    return s:match("^>") ~= nil
end
local function is_ul(s)
    return s:match("^%s*[-*+]%s") ~= nil
end
local function is_ol(s)
...
local function parse_blocks(lines)
    ...
    while i <= #lines do
        ...
        elseif is_ul(line) then
            ...
            while
                i <= #lines
                and (is_ul(peek()) or (peek():match("^  %S") and #items > 0))
            do
                local l = take()
        ...
        elseif is_ol(line) then
            ...
            while
                i <= #lines
                and (is_ol(peek()) or (peek():match("^  %S") and #items > 0))
            do
                local l = take()
            ...
            if #plines > 0 then
                blocks[#blocks + 1] =
                    { type = "para", text = table.concat(plines, " ") }
            end
        end
    end
...
render_blocks = function(blocks)
    ...
    for _, b in ipairs(blocks) do
        ...
        elseif b.type == "code" then
            local cls = b.lang ~= "" and (' class="language-' .. b.lang .. '"')
                or ""
            parts[#parts + 1] = "<pre><code"
                .. cls
                .. ">"
                .. escape(b.text)
                .. "</code></pre>"
        ...
        elseif b.type == "blockquote" then
            ...
            for _, e in ipairs(inner_toc) do
                toc[#toc + 1] = e
            end
