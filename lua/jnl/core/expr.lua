...
local function collect_deps(e, into)
    ...
    -- variadic
    if e.addends then
        for _, child in ipairs(e.addends) do
            collect_deps(child, into)
        end
    end
    if e.factors then
        for _, child in ipairs(e.factors) do
            collect_deps(child, into)
        end
    end
...
---@param field string
local function prime_sym(field)
    return "__prime_" .. field
end
---@param field string
local function expl_sym(field)
    return "__expl_" .. field
end
---@param field string
local function prev_sym(field)
    return "__prev_" .. field
end

---@param name string
local function is_prime(name)
    return name:match("^__prime_(.+)$")
end
---@param name string
...
local function needs_parens(child, parent_prec, is_right_child)
    ...
    if cp < parent_prec then
        return true
    end -- strictly lower: always paren
    if
        cp == parent_prec
        and is_right_child -- same prec, right side of left-assoc
        and ASSOC[child.kind] == "left"
    then
...
function M.pretty(e, parent_prec, is_right_child)
    ...
    elseif e.kind == "div" then
        local p = PREC.div
        return wrap(
            M.pretty(e.a, p, false) .. G.div_op .. M.pretty(e.b, p, true)
        )
    elseif k == "pow" then
...
local function scratch_depth(e)
    ...
    if e._scratch_depth ~= nil then
        return e._scratch_depth
    end
    return 0
end
...
local function walk(e, visitor)
    ...
    if e._walk then
        ...
        for _, key in ipairs({ "addends", "factors" }) do
            for _, child in ipairs(e[key] or {}) do
                walk(child, visitor)
            end
        end
    end
...
local function compile(expr_table, bindings)
    ...
    local function build(e)
        ...
        elseif k == "sym" then
            ...
            assert(
                v ~= nil,
                "expr_binding.compile: no binding for symbol '" .. e.name .. "'"
            )
            if type(v) == "number" then
                return I.const(ud, v)
            end
        ...
        elseif k == "prime" or k == "expl" or k == "prev" then
            ...
            assert(
                v,
                "expr_binding.compile: no binding for '"
        ...
        elseif e._dep_name then
            ...
            assert(
                v,
                "expr_binding.compile: no binding for intermediate '"
                    .. lookup
                    .. "'"
            )
            return I.array(ud, v)
        ...
        error(
            "expr_binding.compile: unhandled expr kind '" .. tostring(k) .. "'"
        )
    end
...
M._doc_subsection = "Construct expressions with add/mul/div/neg/pow and leaves sym/const/cx/cy/cV. "
...
M._api = {
    ...
    collect_deps = {
        args = "e:Expr, into:table?",
        ret = "table<string,true>",
