...
local function module_is_excluded(name, opts)
    ...
    for _, excluded in ipairs(opts.exclude_modules or {}) do
        if name == excluded or starts_with(name, excluded .. ".") then
            return true
        end
    end
...
local function constructor_is_visible(index, constructor, opts)
    ...
    return module and module_is_visible(module, opts)
end
...
local function module_has_content(module)
    ...
    if #(module.value_list or {}) > 0 then
        return true
    end
...
local function resolve_module(index, name)
    if index.raw.modules[name] then
        return index.raw.modules[name]
    end
    local matches = {}
    for module_name, module in pairs(index.raw.modules) do
        if suffix_match(module_name, name) then
    ...
    if #matches == 0 then
        return nil, "unknown documented module: " .. tostring(name)
    end
    local names = {}
    ...
    return nil,
        "ambiguous module '" .. tostring(name) .. "': " .. table.concat(
            names,
            ", "
        )
end
...
local function resolve_type(index, name, module_name)
    if index.raw.types[name] then
        return index.raw.types[name]
    end
    ...
    if type(match) == "string" then
        return index.raw.types[match]
    end
    if type(match) == "table" then
        return nil,
            "ambiguous type '" .. name .. "': " .. table.concat(match, ", ")
    end
...
local function references_for(index, owner_name)
    ...
    for _, ref in ipairs(index.raw.references or {}) do
        if ref.owner == owner_name and ref.resolved then
            refs[#refs + 1] = ref.resolved
        end
    end
    return refs
end
...
local function add_type_closure(index, qualified, selected, recurse)
    if not qualified or selected[qualified] then
        return
    end
    local type_doc = index.raw.types[qualified]
    if not type_doc then
        return
    end
    ...
    for _, method in ipairs(type_doc.methods or {}) do
        for _, referenced in
            ipairs(references_for(index, method.qualified_name))
        do
            add_type_closure(index, referenced, selected, true)
        end
    end
...
local function relevant_types(index, modules, mode)
    ...
    local selected = {}
    local include_local = mode == "local" or mode == "closure"
    ...
    for _, module in ipairs(modules) do
        ...
        if mode ~= "local" then
            for _, symbol in ipairs(module.function_list) do
                if symbol.kind ~= "method" then
                    for _, referenced in
                        ipairs(references_for(index, symbol.qualified_name))
                    do
                        add_type_closure(index, referenced, selected, recurse)
                    end
                end
            end
            ...
            for _, value in ipairs(module.value_list) do
                for _, referenced in
                    ipairs(references_for(index, value.qualified_name))
                do
                    add_type_closure(index, referenced, selected, recurse)
                end
            end
...
local function function_signature(symbol)
    ...
    for _, ret in ipairs(symbol.returns or {}) do
        if ret.type and ret.type ~= "" then
            returns[#returns + 1] = ret.type
        end
    end
    local suffix = #returns > 0 and (" -> " .. table.concat(returns, ", "))
        or ""
...
local function dump_function(p, symbol, indent)
    ...
    if symbol.deprecated then
        local message = symbol.deprecated == true and "deprecated"
            or ("deprecated: " .. symbol.deprecated)
...
local function dump_type(index, p, type_doc, opts)
    ...
    if type_doc.deprecated then
        local message = type_doc.deprecated == true and "deprecated"
    ...
    for _, field in ipairs(type_doc.fields or {}) do
        if field.visibility ~= "private" or opts.include_private then
    ...
    if #constructors > 0 then
        ...
        for _, constructor in
            ipairs(sorted_copy(constructors, "qualified_name"))
        do
...
local function dump_module_body(p, module, opts)
    ...
    if #functions > 0 then
        p:line("   Functions")
        p:blank()
        for _, symbol in ipairs(functions) do
            dump_function(p, symbol, "      ")
        end
    end
...
function Index:package(prefix, opts)
    ...
    for name, module in pairs(self.raw.modules) do
        if
            module_is_visible(module, opts)
            and (name == prefix or starts_with(name, prefix .. "."))
        then
...
function Index:search(query)
    ...
    for name, symbol in pairs(self.raw.symbols) do
        add(symbol.kind or "symbol", name, symbol)
    end
    for name, type_doc in pairs(self.raw.types) do
        add("type", name, type_doc)
    end
    table.sort(matches, function(a, b)
...
function Index:audit(opts)
    ...
    if undocumented_public == nil then
        undocumented_public = true
    end
    local missing_param_types = opts.missing_param_types
    if missing_param_types == nil then
        missing_param_types = strict
    end
    local missing_return_types = opts.missing_return_types
    if missing_return_types == nil then
        missing_return_types = false
    end
            ...
            for _, symbol in ipairs(module.function_list) do
                ...
                then
                    add(
                        "warning",
                        "undocumented-symbol",
                        symbol.qualified_name .. " has no description",
                        symbol
                    )
                end
                ...
                if missing_param_types then
                    for _, param in ipairs(symbol.params or {}) do
                        if
                            param.name ~= "..."
                            and (not param.type or param.type == "")
                        then
                            add(
                                "warning",
                                "untyped-parameter",
                ...
                if missing_return_types and #symbol.returns == 0 then
                    add(
                        "warning",
                        "missing-return",
                        symbol.qualified_name .. " has no return annotation",
                        symbol
                    )
                end
            end
...
function Index:render_package(prefix, opts)
    ...
    p:header(opts.title or ("JNL package: " .. prefix), 1)
