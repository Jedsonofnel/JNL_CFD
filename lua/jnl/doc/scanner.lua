...
local function parse_field(rest)
    ...
    local type_expr, description = parse_named_type_and_doc(type_tail)
...
local function parse_doc_block(raw_lines)
    ...
    for _, raw in ipairs(raw_lines) do
        ...
        if tag then
            ...
            elseif tag == "param" then
                ...
                if param then
                    doc.params[#doc.params + 1] = param
                end
            elseif tag == "return" then
                local ret = parse_return(rest)
                if ret then
                    doc.returns[#doc.returns + 1] = ret
                end
    ...
    while
        #doc.description_lines > 0
        and trim(doc.description_lines[#doc.description_lines]) == ""
    do
...
local function function_decl(line)
    ...
    if local_name then
        return local_name, args, true
    end

    local_name, args =
        line:match("^%s*local%s+([%w_]+)%s*=%s*function%s*%((.-)%)")
    if local_name then
        return local_name, args, true
    end
...
local function attach_params(index, module, symbol, lua_params, annotations)
    local by_name = {}
    for _, param in ipairs(annotations or {}) do
    ...
    for name, param in pairs(by_name) do
        ...
        add_diagnostic(
            ...
            "unknown-parameter",
            string.format(
                "%s documents parameter '%s' not present in its declaration",
...
local function finalize_function(index, module, candidate, class_vars)
    ...
    if not private then
        ...
        if class_type then
            class_type.methods[#class_type.methods + 1] = symbol
        end
    end
...
local function finalize_value(index, module, candidate)
    ...
    if not doc or not doc.type then
        return
    end
    if candidate.is_local and not doc.public then
        return
    end
    if
        not target_is_exported(candidate.target, module.export_root)
        and not doc.public
    then
        return
    end
    ...
    if
        module.export_root and starts_with(relative, module.export_root .. ".")
    then
...
local function extract_type_names(type_expr)
    local names = {}
    local seen = {}
    if not type_expr or type_expr == "" then
        return names
    end

    local scrubbed = type_expr:gsub('".-"', " "):gsub("'.-'", " ")

    for token in scrubbed:gmatch("[%a_][%w_%.]*") do
        if
            not BUILTIN_TYPES[token]
...
local function resolve_type_name(index, module_name, name)
    if BUILTIN_TYPES[name] or TYPE_KEYWORDS[name] then
        return nil, "builtin"
    end
    if index.types[name] then
        return name
    end

    local local_name = module_name .. "." .. name
    if index.types[local_name] then
        return local_name
    end
...
local function collect_type_refs(index, owner, type_expr)
    ...
    for _, name in ipairs(extract_type_names(type_expr)) do
        local resolved, reason, matches =
            resolve_type_name(index, owner.module, name)
        refs[#refs + 1] = {
            name = name,
            resolved = resolved,
            reason = reason,
...
local function link_index(index)
        ...
        for _, ref in ipairs(collect_type_refs(index, owner, type_expr)) do
            ...
            if ref.reason == "unresolved" then
                add_diagnostic(
                    ...
                    "unresolved-type",
                    string.format(
                        "%s references unresolved type '%s'",
            ...
            elseif ref.reason == "ambiguous" then
                add_diagnostic(
                    ...
                    "ambiguous-type",
                    string.format(
                        "%s references ambiguous type '%s': %s",
                        owner.qualified_name,
                        ref.name,
                        table.concat(ref.matches, ", ")
                    ),
                    describe_location(owner.module, owner.path, owner.line)
                )
            end
    ...
    for _, module in pairs(index.modules) do
        for _, symbol in ipairs(module.function_list) do
            ...
            for i, ret in ipairs(symbol.returns) do
                ...
                if symbol.kind == "function" then
                    for _, ref in
                        ipairs(collect_type_refs(index, symbol, ret.type))
                    do
                        local type_doc = ref.resolved
                                and index.types[ref.resolved]
                            or nil
                        if type_doc and type_doc.kind == "class" then
                            type_doc.constructors[#type_doc.constructors + 1] =
                                symbol
                        end
                    end
                end
        ...
        for _, type_doc in ipairs(module.type_list) do
            if type_doc.alias_type then
                link_owner(type_doc, type_doc.alias_type, "alias")
            end
            ...
            if target then
                ...
                if table_name then
                    ...
                    elseif table_local then
                        module.locals[table_name] =
                            { kind = "table", line = line_no }
                    end
                else
...
local function default_list_files(dir)
    local pipe = io.popen(
        "find "
            .. shell_quote(dir)
            .. " -type f -name '*.lua' -print 2>/dev/null"
    )
    if not pipe then
        return {}, "could not launch find"
    end
    local files = {}
...
local function module_name_from_path(root, path)
    ...
    if starts_with(relative, "/") then
        relative = relative:sub(2)
    end
    if not ends_with(relative, ".lua") then
        return nil
    end
    relative = relative:sub(1, -5)
    if ends_with(relative, "/init") then
        relative = relative:sub(1, -6)
    end
...
function M.discover_package(package_name, opts)
    ...
    for _, root in ipairs(path_roots(package_path)) do
        ...
        for _, path in ipairs(files or {}) do
            ...
            if
                module_name
                and (module_name == package_name or starts_with(
                    module_name,
                    package_name .. "."
                ))
...
function M.scan(opts)
    ...
    for _, module_name in ipairs(names) do
        local path, resolve_err =
            M.resolve_module(module_name, index.package_path)
