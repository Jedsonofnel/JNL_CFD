...
local solver_ctors = {
    cg_jac = function(sys, phi, tol, _)
        return sys:cg_jac(phi, tol)
    end,
    cg_dic = function(sys, phi, tol, _)
        return sys:cg_dic(phi, tol)
    end,
    bicgstab_jac = function(sys, phi, tol, _)
        return sys:bicgstab_jac(phi, tol)
    end,
    bicgstab_dilu = function(sys, phi, tol, _)
        return sys:bicgstab_dilu(phi, tol)
    end,
    gmres_dilu = function(sys, phi, tol, opts)
        return sys:gmres_dilu(phi, tol, opts.restart or 20)
    end,
}
