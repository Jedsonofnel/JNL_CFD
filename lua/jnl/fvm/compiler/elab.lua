...
local function list_add(t, value)
	if not value or list_has(t, value) then return end
	t[#t + 1] = value
end
...
local function append_all(dst, src)
	for _, v in ipairs(src or {}) do list_add(dst, v) end
end
...
local function elab_add_inv(elab, source, iname)
	if not source or not iname then return end
...
local function collect_deps_from_node(node, deps)
	...
	if node.kind == "component" and node.a and node.a.kind == "symbol" and node.a.name then
		local idx = node.b and node.b.a
		local ax  = AXES[idx]
		if ax then list_add(deps, Mangle.field(node.a.name, ax)) end
	end
...
local function elab_add_diag(elab, field, parent)
	...
	if parent then elab_add_inv(elab, parent, dname) end
...
local function elab_add_mwi(elab, reg, mwi_node)
	...
	if elab.fields[mname] and elab.face_flux[mname] then return mname end

	local Uentry = reg:entry(Uname)
	local comps  = (Uentry and Uentry.rank == 1) and comp_names(Uname) or { Uname }
...
local function elab_flux_for(elab, reg, flux_node, counter)
	assert(flux_node and Node.is_node(flux_node),
		"elab_flux_for: expected rank-1 Node")
	assert(flux_node.rank == 1,
		"elab_flux_for: expected rank-1 flux, got rank-" .. tostring(flux_node.rank))
...
local function implicit_flux_child(node)
	...
	if node.kind == "mwi" then return node end

	if node.kind == "scale" then
		if node.a and node.a.kind == "mwi" then return node.a end
		if node.b and node.b.kind == "mwi" then return node.b end
	end
...
local function elab_add_div_cell(elab, reg, div_node, counter)
	...
	if flux_entry then append_all(deps, flux_entry.deps) end
	...
	if k == "divergence" then
		...
		if mode == "expr" then
			...
			if inner.kind == "outer" then
				...
				elab_flux_for(elab, reg, implicit_flux_child(inner), elab.counter)
			end
		end
