...
local function neg_node(n)
    if n.kind == "constant" then
        return Node.const(-n.a)
    end
    if n.kind == "neg" then
        return n.a
    end
...
local function resolve_div_cells(node, ctx)
    if not node or type(node) ~= "table" or not node.kind then
        return node
    end
    ...
    if new_a == node.a and new_b == node.b then
        return node
    end
...
local function classify_coeff(node, sign, ctx)
    ...
    if ctx then
        n = resolve_div_cells(n, ctx)
    end

    if n.kind == "constant" then
        return "k", n
    end
    if n.kind == "symbol" then
        return "f", n
    end
    return "expr", n
end
...
local function extract_scale(node, op)
    ...
    if node.kind == "scale" or node.kind == "mul" then
...
local function resolve_eval_components(ctx, node)
    local r = Resolve.resolve(node, ctx.ndims)
    assert(
        type(r) == "table" and not Node.is_node(r),
...
local function emit_div_source(out, field, div_node, sign, ctx, fresh)
    ...
    assert(
        flux_name,
        "lower: no face flux for divergence source " .. tostring(div_node.a)
    )
...
local function emit_div_cell_inst(out, dname, entry, ctx)
    ...
    elseif entry.flux_kind == "expr" and flux_entry then
        ...
        local inst =
            Inst.divergence_c(dname, flux_entry.vec_x, flux_entry.vec_y, true)
        if ctx.ndims == 3 then
            inst.uz = flux_entry.vec_z
        end
        out[#out + 1] = inst
    elseif entry.flux_kind == "mwi" and flux_entry then
        ...
        error(
            string.format(
                "div_cell: unsupported flux kind '%s' for '%s'",
                tostring(entry.flux_kind),
                dname
            )
        )
    end
...
walk_scalar.divergence = function(out, node, field, sign, ctx, fresh)
    ...
    if
        inner.kind == "scale"
        and inner.b.kind == "mwi"
        and inner.a.name == field
    then
...
walk_vector.grad = function(out, node, _, comps, sign, ctx)
    ...
    for i, comp in ipairs(comps) do
        emit_su_field_comp(
            out,
            comp,
            Mangle.grad(scalar.name, ctx.axes[i]),
            sign,
            node
        )
    end
...
abstract_expand.correct = function(inst, ctx)
    local entry = ctx.reg:entry(inst.field)
    if not entry or not entry.correction then
        return {}
    end
...
local function lower_phase(phase, ctx)
    ...
    for _, inst in ipairs(phase) do
        ...
        if fn then
            ...
            for _, ci in ipairs(concrete) do
                out[#out + 1] = ci
            end
        else
            out[#out + 1] = inst
...
function M.lower_equation(field, entry, elab, ndims, reg)
    assert(
        entry.equation,
        "lower_equation: entry '" .. field .. "' has no equation"
    )
    ...
    elseif entry.rank == 1 then
        ...
        error(
            "lower_equation: rank-" .. tostring(entry.rank) .. " not supported"
        )
    end
    ...
    for _, inst in ipairs(out) do
        ...
        if inst.op:find("^patch_") then
            has_ghost_fills = true
        end
    end
