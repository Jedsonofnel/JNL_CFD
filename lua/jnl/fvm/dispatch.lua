...
local function coeff_vec(case, c)
    ...
    if c == "__coeff" then
        assert(case.exec.coeff, "dispatch: __coeff read before eval_coeff ran")
...
D.lap_nonorth_k = function(case, inst)
    if not case.cfg:get(inst.field, "non_ortho") then
        return
    end
    B.laplacian_nonorth_k(
        sys(case, inst.field),
        case.mesh,
        inst.coeff,
...
D.div_dc = function(case, inst)
    if case.cfg:get(inst.field, "div") ~= "tvd" then
        return
    end
...
D.su_fs = function(case, inst)
    local src = inst.src == "__coeff" and case.exec.coeff
        or field(case, inst.src)
