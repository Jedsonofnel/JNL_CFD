...
function Inst.evaluate(field, implicit)
    ...
    return new(
        "evaluate",
        { field = field, implicit = implicit, level = "abstract" }
    )
end
...
function Inst.face_normal(ux_face, uy_face, out)
    return new(
        "face_normal",
        { ux_face = ux_face, uy_face = uy_face, out = out }
    )
end
...
function Inst.div_f(field, flux, coeff, expr)
    return new(
        "div_f",
        { field = field, flux = flux, coeff = coeff, node = expr }
    )
end
...
function Inst.div_dc(field, flux, grad_x, grad_y)
    return new(
        "div_dc",
        { field = field, flux = flux, grad_x = grad_x, grad_y = grad_y }
    )
end
...
function Inst.su_k(field, coeff, volumetric)
    return new(
        "su_k",
        { field = field, coeff = coeff, volumetric = volumetric }
    )
end
...
function Inst.su_f(field, coeff, volumetric, expr)
    return new(
        "su_f",
        { field = field, coeff = coeff, volumetric = volumetric, node = expr }
    )
end
...
function Inst.pfill_s_d(field, patch, value)
    return new(
        "patch_s_fill_d",
        { field = field, patch = patch, value = value }
    )
end
...
function Inst.pfill_s_n(field, patch, grad_n)
    return new(
        "patch_s_fill_n",
        { field = field, patch = patch, grad_n = grad_n }
    )
end
...
function Inst.pfill_s_r(field, patch, a, b, c)
    return new(
        "patch_s_fill_r",
        { field = field, patch = patch, a = a, b = b, c = c }
    )
end
...
function concrete_fmt.divergence(f, _)
    ...
    return string.format("DIVERGENCE %-4s %s -> %s", mode, f.flux, f.out)
end

function concrete_fmt.divergence_c(f, _)
    local mode = f.integrated == false and "[v]" or "[i]"
    return string.format(
        "DIVERGENCE_C %-4s (%s,%s) -> %s",
        mode,
        f.ux,
        f.uy,
        f.field
...
function concrete_fmt.lap_nonorth_k(f, cfg)
    if not cfg:get(f.field, "non_ortho") then
        return nil
    end
    return string.format(
        "LAP_NONORTH_K %s  gamma=%g  grad=(%s,%s)",
        f.field,
        f.coeff,
        f.grad_x,
        f.grad_y
...
function concrete_fmt.lap_nonorth_f(f, cfg)
    ...
    return string.format(
        "LAP_NONORTH_F %s  gamma=%s  grad=(%s,%s)",
        f.field,
        coeff_str(f),
        f.grad_x,
        f.grad_y
...
function concrete_fmt.div_dc(f, cfg)
    if cfg:get(f.field, "div") ~= "tvd" then
        return nil
    end
    return string.format(
        "DIV_DC        %s  flux=%s  grad=(%s,%s)",
        f.field,
        f.flux,
        f.grad_x,
        f.grad_y
