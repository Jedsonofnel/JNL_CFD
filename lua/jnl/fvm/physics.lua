-- fvm/physics.lua - physics case that gets compiled
...
--
-- Physics - manages algorithm and registry and performs compilation
--

local Physics = {}
Physics.__index = Physics

Physics._deepcopy = deepcopy -- dirty but not so bad - can move to core.util later

function Physics.new(registry, algorithm)
...
function Physics:make_case(mesh, bcs)
	local Case = require("jnl.fvm.case")
	return Case.new(self, mesh, bcs)
end

function Physics:print_registry()
	self.registry:print()
end

function Physics:print_algorithm()
	self.algorithm:print()
end

function Physics:print_instructions()
	print(compile.instruction_listing(self.instructions, self.post_instructions))
end

function Physics:print_resources()
...
function Physics:_expand_intermediates()
