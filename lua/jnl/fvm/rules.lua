...
function M.residual_below(field, threshold, n_consec)
    ...
    assert(
        ...
        "residual_below: threshold must be a number"
    )
    return {
        kind = "conv_crit",
        field = field,
...
function M.change_below(field, threshold, n_consec)
    ...
    assert(
        type(threshold) == "number",
        "change_below: threshold must be a number"
    )
    return {
        kind = "conv_crit",
        field = field,
...
eval_conv.residual_below = function(sage, field, crit)
    ...
    if #h < crit.n_consec then
        return false
    end
    for _, f in ipairs(h) do
        if f.value >= crit.threshold then
            return false
        end
    end
    return true
            ...
            -- ALL active conv_crit must be satisfied for the solver to converge.
            {
                ...
                fire = function(sage, f)
                    ...
                    for _, c in ipairs(expanded) do
                        ...
                        if not fn then
                            io.stderr:write(
                                "rules: unknown conv op '"
                                    .. tostring(c.op)
                                    .. "'\n"
                            )
                            return
                        end
                        if not fn(sage, c.field, c) then
                            return
                        end
                    end
