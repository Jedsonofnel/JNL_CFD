...
function FvmStudy:default_evaluate(x, opts)
    ...
    return setmetatable(res, {
        __tostring = function(r)
            local parts = {}
            local names = r.sim.diag.system_names and r.sim.diag.system_names()
                or {}
            ...
            for _, name in ipairs(names) do
                ...
                if resid then
                    parts[#parts + 1] =
                        string.format("%s=%.3e", E.pretty_sym(name), resid)
                end
            end

            table.sort(parts)

            local res_str = #parts > 0 and table.concat(parts, "  ")
                or "no residuals"
...
function FvmStudy:install(repl)
    ...
    if self.geometry_fn then
        self:expose(
            ...
            "Build the PSLG geometry for the current design",
            { hidden = true }
        )
    ...
    if
        self.case_fn
        or self.mesh_fn and self.registry_fn and self.algorithm_fn
    then
