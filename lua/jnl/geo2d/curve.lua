...
function M.circle(centre, radius, opts)
    ...
    local arcs = {}
    for i = 0, 3 do
        arcs[i + 1] = I.arc(cx, cy, radius, a0 + i * da, a0 + (i + 1) * da)
    end
...
function M.through(points)
    assert(
        type(points) == "table" and #points >= 2,
...
function M.closed_polyline(points, eps)
    assert(
        type(points) == "table" and #points >= 3,
...
function M.map(curve, fn, opts)
    ...
    local pts =
        curve:sample(n, opts.distribution or I.uniform(), opts.mode or "arclen")
...
function M.discretise_onto(curve, pslg, marker, opts)
    ...
    local nodes = {}
    for i, p in ipairs(points) do
        nodes[i] = pslg:node_find_or_add(p[1], p[2], 0, opts.eps or 1e-10)
    end
