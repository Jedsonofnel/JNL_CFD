...
local function apply_free_edges(raw_blk, edges, skip)
    for edge, spec in pairs(edges) do
        if not skip[edge] then
            if spec.marker then
                raw_blk:set_edge_marker(edge, spec.marker)
            end
...
local function check_block_edges(h, idx, targets)
    for _, edge in ipairs(ALL_EDGES) do
        ...
        if not assigned and not targeted then
            return ("block %d: edge '%s' is neither assigned nor a join target"):format(
                idx,
                EDGE_NAME[edge]
            )
        end
        ...
        if assigned and targeted then
            return (
                "block %d: edge '%s' is a join target and must not be assigned "
...
function BlockBuilder:build()
    ...
    if self.smooth_opts then
        blk:smooth(self.smooth_opts)
    end
...
local function edge_npoints(h, edge)
    if edge == S.SOUTH or edge == S.NORTH then
        return h.ni
    end
    return h.nj
end
...
---@return BlockBuilder
function BlockBuilder:south(c, opts)
    return set_edge(self, S.SOUTH, c, opts)
end

---@param c     Curve2D
---@param opts? { marker:integer?, dist:Dist1D? }
---@return BlockBuilder
function BlockBuilder:east(c, opts)
    return set_edge(self, S.EAST, c, opts)
end

---@param c     Curve2D
---@param opts? { marker:integer?, dist:Dist1D? }
---@return BlockBuilder
function BlockBuilder:north(c, opts)
    return set_edge(self, S.NORTH, c, opts)
end

---@param c     Curve2D
---@param opts? { marker:integer?, dist:Dist1D? }
---@return BlockBuilder
function BlockBuilder:west(c, opts)
    return set_edge(self, S.WEST, c, opts)
end
...
---@param opts? { marker:integer?, dist:Dist1D? }
---@return BlockBuilder
function BlockBuilder:edge(edge, c, opts)
    return set_edge(self, edge, c, opts)
end
...
function GridBlockHandle:south(c, opts)
    return set_edge(self, S.SOUTH, c, opts)
end
...
---@param opts? { marker:integer?, dist:Dist1D? }
---@return GridBlockHandle
function GridBlockHandle:east(c, opts)
    return set_edge(self, S.EAST, c, opts)
end
...
---@param opts? { marker:integer?, dist:Dist1D? }
---@return GridBlockHandle
function GridBlockHandle:north(c, opts)
    return set_edge(self, S.NORTH, c, opts)
end
...
---@param opts? { marker:integer?, dist:Dist1D? }
---@return GridBlockHandle
function GridBlockHandle:west(c, opts)
    return set_edge(self, S.WEST, c, opts)
end
...
---@param opts? { marker:integer?, dist:Dist1D? }
---@return GridBlockHandle
function GridBlockHandle:edge(edge, c, opts)
    return set_edge(self, edge, c, opts)
end
...
local function prepare_raw_grid(self)
    ...
    if corner_err then
        return nil, nil, corner_err
    end

    local npoints_err = check_join_npoints(self.joins)
    if npoints_err then
        return nil, nil, npoints_err
    end
    ...
    for _, h in ipairs(self.blocks) do
        ...
        if h.smooth_opts then
            h.raw:smooth(h.smooth_opts)
        end
    end
