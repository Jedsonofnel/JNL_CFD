...
local function needs_parens(child, parent_prec, is_right_child)
    ...
    if cp == parent_prec and is_right_child and ASSOC[child.kind] == "left" then
        return true
    end
    return false
end
...
node_pretty = function(node, parent_prec, is_right)
    ...
    elseif k == "scale" then
        ...
        local sep = node.a:is_anon_const() and node.b.kind == "symbol" and ""
            or G.mul
