...
local function sum_nodes(nodes)
    ...
    for i = 2, #nodes do
        result = Node.add(result, nodes[i])
    end
    return result
end
...
local function inv_3x3(comp)
    ...
    local C = {
        {
            minor2(2, 2, 3, 3),
            Node.negate(minor2(2, 1, 3, 3)),
            minor2(2, 1, 3, 2),
        },
        {
            Node.negate(minor2(1, 2, 3, 3)),
            minor2(1, 1, 3, 3),
            Node.negate(minor2(1, 1, 3, 2)),
        },
        {
            minor2(1, 2, 2, 3),
            Node.negate(minor2(1, 1, 2, 3)),
            minor2(1, 1, 2, 2),
        },
    }
    local det = sum_nodes({
...
local resolve_scalar, resolve_component, resolve_component_2d, resolve_all_1d, resolve_all_2d

resolve_all_1d = function(node, ndims)
    local out = {}
    for i = 1, ndims do
        out[i] = resolve_component(node, i, ndims)
    end
    return out
end
...
resolve_all_2d = function(node, ndims)
    ...
    for i = 1, ndims do
        out[i] = {}
        for j = 1, ndims do
            out[i][j] = resolve_component_2d(node, i, j, ndims)
        end
    end
    return out
end
...
resolve_scalar = function(node, ndims)
    ...
    if acc then
        ...
        assert(
            node.rank == 0,
            string.format(
                "resolve_scalar: accessor '%s' is rank-%d, expected rank-0",
                k,
                node.rank
            )
        )
    ...
    if k == "dot" then
        ...
        local terms = {}
        for i = 1, ndims do
            terms[i] = Node.multiply(
                resolve_component(node.a, i, ndims),
    ...
    if k == "trace" then
        ...
        local terms = {}
        for i = 1, ndims do
            terms[i] = resolve_component_2d(node.a, i, i, ndims)
        end
        return sum_nodes(terms)
    end
    ...
    if k == "ddt" or k == "laplacian" then
        assert(
            node.a.rank == 0,
            string.format(
                "resolve_scalar: %s expects rank-0 operand, got rank-%d",
                k,
                node.a.rank
            )
        )
        ...
        if node.b then
            n.b = resolve_scalar(node.b, ndims)
        end
        return mk(n)
    end
...
resolve_component = function(node, i, ndims)
    assert(
        i >= 1 and i <= ndims,
        string.format(
            "resolve_component: index %d out of range [1,%d]",
            i,
            ndims
        )
    )
    ...
    if node.rank == 0 then
        return resolve_scalar(node, ndims)
    end

    assert(
        node.rank == 1,
        string.format(
            "resolve_component: expected rank-1, got rank-%d (kind=%s)",
            node.rank,
            node.kind
        )
    )
    ...
    if k == "cross" then
        ...
        if i == 1 then
            return Node.subtract(
                Node.multiply(
                    resolve_component(a, 2, ndims),
                    resolve_component(b, 3, ndims)
                ),
                Node.multiply(
                    resolve_component(a, 3, ndims),
                    resolve_component(b, 2, ndims)
                )
            )
        elseif i == 2 then
            return Node.subtract(
                Node.multiply(
                    resolve_component(a, 3, ndims),
                    resolve_component(b, 1, ndims)
                ),
                Node.multiply(
                    resolve_component(a, 1, ndims),
                    resolve_component(b, 3, ndims)
                )
            )
        else
    ...
    if k == "grad" then
        ...
        assert(
            node.a.kind == "symbol" and node.a.name,
...
resolve_component_2d = function(node, i, j, ndims)
    assert(
        node.rank == 2,
        string.format(
            "resolve_component_2d: expected rank-2, got rank-%d (kind=%s)",
            node.rank,
            node.kind
        )
    )
    ...
    if k == "matmul" then
        ...
        local terms = {}
        for kk = 1, ndims do
            terms[kk] = Node.multiply(
                resolve_component_2d(node.a, i, kk, ndims),
    ...
    if k == "grad" then
        assert(node.a.rank == 1, "resolve_component_2d: grad of non-vector")
        local Ui = resolve_component(node.a, i, ndims)
        assert(
            Ui.kind == "symbol" and Ui.name,
...
function M.resolve(node, ndims)
    ...
    if node._requires_3d and ndims == 2 then
        error(
            string.format(
                "resolve(2): expression contains '%s' which requires ndims=3",
                node.kind
            ),
            2
        )
