...
local function build_mul(product, exps, base_of, order, others)
    local parts = {}
    if product ~= 1 then
        parts[#parts + 1] = Node.const(product)
    end
    ...
    for _, name in ipairs(order) do
        ...
        if e and not e:is_zero() then
            if e:is_one() then
                ...
                parts[#parts + 1] = setmetatable(
                    { kind = "pow", a = base_of[name], b = e, rank = 0 },
                    Node
                )
            end
        end
...
local function simplify_add(a, b)
    local terms =
        Node.flatten(setmetatable({ kind = "add", a = a, b = b }, Node), "add")
    local sum = 0
    local non_const = {}
...
local function simplify_mul(a, b)
    local factors =
        Node.flatten(setmetatable({ kind = "mul", a = a, b = b }, Node), "mul")
    local product = 1
    local exps, base_of, order, others = {}, {}, {}, {}
...
local function simplify_div(a, b)
    ...
    return setmetatable(
        { kind = "div", a = num, b = den, rank = num.rank },
        Node
    )
end
