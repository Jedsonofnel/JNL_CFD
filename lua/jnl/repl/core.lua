...
local function fennel_view(repl, value, opts)
    ...
    local ok, rendered = pcall(view, value, opts or fennel_view_opts(repl))
...
local function install_completion(repl)
    local ok, host = pcall(require, "jnl.repl.host")

    if not ok or type(host.set_completer) ~= "function" then
        return nil
    end

    host.set_completer(function(context)
        return Completion.complete(repl, context)
    end)
...
function Repl:register(name, value, doc)
    ...
    local description = Help.registration_doc(self, name, doc)
...
function Repl:command(name, fn, usage, doc)
    ...
    assert(type(fn) == "function", "repl.command: callback must be a function")
...
function Repl:usage_string()
    ...
    if type(spec) == "table" and type(spec.string) == "function" then
        return spec:string()
    end

    if type(spec) == "table" and type(spec.usage_string) == "function" then
