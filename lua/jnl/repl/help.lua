...
local function find_item(raw, values, reference, include_private)
	...
	for name, item in pairs(values) do
		if suffix_match(name, reference)
			and (
				include_private
				or module_is_public(raw, item)
			)
		then
...
function M.print_overview(repl)
	...
	for _, name in ipairs(
		append_sorted_keys({}, repl.commands)
	) do
	...
	printer:bullet(
		"Fennel results are available as *1, *2, and *3."
	)
	printer:bullet(
		"Named specials may be stored as *name* with remember."
	)
	printer:bullet(
		"Ctrl-C at the prompt clears the current line."
	)
	printer:bullet(
		"Ctrl-C once requests cancellation; twice forces it."
	)
	printer:bullet(
...
function M.print_topic(repl, name)
	...
	if M.is_result_name(name)
		or M.is_special_name(name)
	then
		printer:line(string.format(
			"no help for '%s'",
			name
		))
		return
