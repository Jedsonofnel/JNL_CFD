...
local function format_overrides(design_defaults, overrides)
    ...
    for k, v in pairs(overrides) do
        ...
        if not DISPLAY_IGNORE_KEYS[k] and design_defaults[k] ~= v then
            parts[#parts + 1] =
                string.format("%s=%s", tostring(k), format_value(v))
        end
    end
...
local function format_derived(study, x)
    ...
    for _, item in ipairs(study.derived_inputs or {}) do
        if not item.hidden then
            ...
            if value ~= nil then
                parts[#parts + 1] =
                    string.format("%s=%s", item.label, format_value(value))
            end
        end
    end
...
local function display_name(name)
    return tostring(name):gsub("_", "-"):gsub("-", " ")
end
...
local function auto_doc(kind, name)
    ...
    if kind == "figure" then
        return "Plot or write "
            .. label
            .. " as .csv, .png, .svg, .pdf, or .eps"
    end
...
local function collect_scalar_fields(out, source, prefix)
    for k, v in pairs(source or {}) do
        if is_scalar(v) then
            local name = prefix and (prefix .. "." .. tostring(k))
                or tostring(k)
            out[name] = v
        end
    end
...
function Study:figure_workflow(name, figure_fn, opts)
    ...
    self:expose(
        ...
        "Return the last " .. display_name(workflow_name) .. " figure",
        { hidden = true }
    )

    self:expose(
        "last-" .. workflow_name .. "-arg",
        function()
            return state.arg
        end,
        "Return the last "
...
function Study:usage_string()
    ...
    if self.evaluate_fn then
        p:columns(
            "  (evaluate)",
            self.evaluate_meta.doc or "Evaluate the default design"
        )
        p:columns(
            "  (evaluate {...})",
            "Evaluate with design-variable overrides"
        )
    ...
    for _, item in ipairs(self.outputs) do
        p:columns(
            "  (" .. item.name .. ")",
            item.doc or "Return output from the last result"
        )
    end
    for _, item in ipairs(self.plots) do
        p:columns(
            "  (plot-" .. item.name .. ")",
            item.doc or "Plot from the last result"
        )
    end
    for _, item in ipairs(self.writers) do
        p:columns(
            "  (write-" .. item.name .. ' "path")',
            item.doc or "Write from the last result"
        )
    end
    for _, item in ipairs(self.optimisers) do
        p:columns(
            "  (" .. item.name .. ")",
            item.doc or "Run optimisation helper"
        )
    end
    ...
    p:blank()
    p:line(
        "Inputs: use (default-design), (bounds), and (defaults) to inspect available variables."
    )
...
function Study:record_model(base, spec)
    ...
    return function(sample, i, n)
        ...
        local heading =
            format_model_heading(spec.heading, sample, i, n or spec.n)
        if heading then
...
function Study:install(repl)
    ...
    if self.evaluate_fn then
        ...
        repl:register(
            "evaluate",
            run,
            self.evaluate_meta.doc or "Evaluate the study"
        )
        ...
        if #self.metrics > 0 then
            ...
            repl:register("write-metrics-table", function(path, result)
                assert(
                    type(path) == "string",
                    "write-metrics-table requires a path"
                )
        ...
        repl:register(
            ...
            "Return completed scalar cache record for inputs, or nil if absent/incomplete"
        )
        ...
        repl:register(
            ...
            "Return cached non-scalar data by name for the given design, or nil"
        )
        ...
        repl:register(
            ...
            "Ensure completed scalar cache record exists; run if absent, skip if cached"
        )
        ...
        repl:register(
            ...
            "Return completed cache records matching defaults plus query.where, allowing query.vary"
        )
        ...
        repl:register(
            ...
            "Return x/y arrays from matching cache records; query requires x and y"
        )
    ...
    Study = {
        ...
        methods = {
            ...
            derived = {
                ...
                ret = "Study",
                doc = "Register a derived input computed before evaluation and added to the design table; opts: { doc, hidden|quiet }",
            },
            apply_derived = {
            ...
            print_metrics = {
                args = "result:table",
                ret = "Study",
                doc = "Print scalar result metrics using metric_columns when available; intended for quiet-respecting run summaries.",
            },
            -- Caching
            start_record = {
                args = "design:table, opts:table",
            ...
            cache = {
                args = "",
                ret = "table",
                doc = "Return chronological scalar cache records. Each record has status: pending, done, diverged, or error; default queries only use done records.",
            },
            clear_cache = {
                args = "",
                ret = "Study",
            ...
            keep = {
                ...
                ret = "any",
                doc = "Keep a workflow value by name. Stores it internally, exposes it as a REPL special "
            ...
            output = {
                ...
                ret = "Study",
                doc = "Register an output helper over the last result; fn_or_path defaults to a result key derived from name and may be a dotted result path",
            },
            plot = {
            ...
            figure = {
                ...
                ret = "Study",
                doc = "Register matching plot and writer helpers from one figure factory; opts: { doc, plot_doc, write_doc, write }",
            },
            figure_workflow = {
                args = "name:string, figure_fn:function(arg, ctx)->Figure, opts:table?",
                ret = "Study",
                doc = "Register matching plot and writer helpers for a workflow figure. "
            ...
            table = {
                ...
                ret = "Study",
                doc = "Register matching output and CSV writer helpers from one table factory; table_fn returns { columns, rows }; opts: { doc, output_doc, write_doc }",
            },
            sweep = {
                args = "name:string, fn:function(study, arg), opts:table?",
                ret = "Study",
                doc = "Register a parameter sweep. fn receives the study and an optional argument from the REPL call; "
            ...
            optimise = {
                args = "name:string, fn:function(study, arg), opts:table?",
                ret = "Study",
                doc = "Register an optimisation helper. fn receives the study and an optional REPL argument; "
            ...
            run = {
                args = "design_overrides:table?",
                ret = "table",
                doc = "Evaluate the study and update the scalar cache record; design_overrides may include "
                    .. "display-only options { quiet, heading, print_metrics }. Stores result as last_result.",
            },
            ensure_record = {
                args = "design_overrides:table?",
                ret = "table",
                doc = "Ensure a completed scalar cache record exists for inputs. Display-only options such as "
            ...
            record_for = {
                args = "base:table?, overrides:table?",
                ret = "table",
                doc = "Return ensure_record(with_base(base, overrides)); convenience helper for sweeps, UQ, and optimisation closures",
            },
            record_model = {
            ...
            query_records = {
                args = "query:table?",
                ret = "table",
                doc = "Return scalar cache records matching defaults plus query.where. "
            ...
            query_xy = {
                ...
                doc = "Return x/y arrays from matching scalar cache records. "
