...
h.describe("attribute serialisation", function()
    h.it("attrs with a string child", function()
        h.expect(H.a({ href = "/" })("Home")).equals('<a href="/">Home</a>')
    end)
    ...
    h.it("attrs with nil child produces an empty element", function()
        h.expect(H.div({ class = "x" })()).equals('<div class="x"></div>')
    end)

    h.it(
        "attr keys are sorted alphabetically for deterministic output",
        function()
    ...
    h.it("false value omits the attribute entirely", function()
        h.expect(H.p({ class = false })("x")).equals("<p>x</p>")
    end)
end)
...
h.describe("void elements", function()
    ...
    h.it("attrs produce a self-closing tag with attributes", function()
        h.expect(H.input({ type = "text" })).equals('<input type="text" />')
    end)
...
h.describe("string formatting", function()
    ...
    h.it("extra arguments trigger string.format", function()
        h.expect(H.p("item %d of %d", 3, 10)).equals("<p>item 3 of 10</p>")
    end)
...
h.describe("conditional rendering", function()
    ...
    h.it(
        "when does not call a function child when condition is falsy",
        function()
            local called = false
    ...
    h.it(
        "when calls a function child and renders result when truthy",
        function()
...
h.describe("fragments", function()
    h.it("fragment concatenates nodes without a wrapper element", function()
        h.expect(H.fragment(H.p("a"), H.p("b"))).equals("<p>a</p><p>b</p>")
    end)
