...
h.describe("paragraphs", function()
	...
	h.it("soft line breaks within a paragraph are joined with a space", function()
		local html = md.render("Line one\nLine two\n")
		h.expect(html:find("Line one Line two")).is_not_nil()
	end)

	h.it("multiple consecutive blank lines do not produce extra elements", function()
...
h.describe("unordered lists", function()
	...
	h.it("indented items produce a nested sub-list inside the parent li", function()
