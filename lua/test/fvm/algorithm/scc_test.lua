...
local function check_scc_structure(reg, label)
    h.describe(label .. ": SCC detection", function()
        ...
        h.it(
            "no coupled groups (velocity-pressure coupling is via corrections, not equations)",
            function()
                h.expect(#result.coupled).equals(0)
            end
        )
        ...
        h.it(
            "inv_d does not appear in any group (diagnostic, not prognostic)",
            function()
                for _, g in ipairs(result.groups) do
        ...
        h.it(
            "p does not appear in any group (has no governing equation)",
            function()
                for _, g in ipairs(result.groups) do
...
h.describe("preset: stokes vs ns structural differences", function()
    ...
    h.it(
        "ns U equation reads U via convective term (stokes does not)",
        function()
    ...
    h.it(
        "p_prime equation rhs has opposite sign between stokes and ns",
        function()
