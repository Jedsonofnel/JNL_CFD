...
local function make_ns_reg()
    ...
    omega:governed_by(
        (nb.ddt(omega) + nb.div(nb.mwi(U, p) * omega)):equals(
...
local function ops_in(phase, op_name)
    ...
    for _, inst in ipairs(phase) do
        if inst.op == op_name then
            out[#out + 1] = inst.field
        end
    end
    return out
end
...
local function abs_pos(phase, op_name, field)
    for i, inst in ipairs(phase) do
        if inst.op == op_name and inst.field == field then
            return i
        end
    end
    return nil
...
local function abs_next(phase, op_name, field, after)
    for i = after + 1, #phase do
        if phase[i].op == op_name and phase[i].field == field then
            return i
        end
    end
    return nil
...
h.describe("expand: PRE fills", function()
    ...
    h.it("fills are emitted in alphabetical order", function()
        local fills = ops_in(alg.pre, "fill")
        for i = 2, #fills do
            h.expect(fills[i] >= fills[i - 1])
                .is_truthy("out of order: " .. fills[i - 1] .. " before " .. fills[i])
        end
    end)
...
h.describe("expand: invalidation after SOLVE U", function()
    ...
    h.it(
        "nut is not spuriously re-evaluated between SOLVE U and ZERO p_prime",
        function()
            ...
            h.expect(nut_pos == nil or nut_pos > zero_pos)
                .is_truthy("nut spuriously re-evaluated after SOLVE U")
        end
    )
end)
...
h.describe("expand: clip instruction placement", function()
    ...
    h.it("omega clip immediately follows SOLVE omega", function()
        local omg_pos = abs_pos(alg.main, "solve", "omega")
        h.expect(abs_next(alg.main, "clip", "omega", omg_pos))
            .equals(omg_pos + 1)
    end)
