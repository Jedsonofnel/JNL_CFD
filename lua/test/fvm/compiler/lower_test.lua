...
local function make_div_sym_rhs_bare()
    ...
    phi:governed_by(nb.laplacian(phi):equals(nb.div(U)))
...
local function make_div_expr_rhs()
    ...
    phi:governed_by(nb.laplacian(phi):equals(nb.div(U + W)))
...
local function op_f(name, f)
    return function(i)
        return i.op == name and i.field == f
    end
end
...
h.describe("lower: scalar solve instruction ordering", function()
    ...
    h.it(
        "sys_reset -> lap_k -> lap_nonorth_k -> under_relax -> solve_linalg",
        function()
...
h.describe("lower: divergence paired with deferred correction", function()
    ...
    h.it("div_k uses mwi flux name", function()
        ...
        for _, inst in ipairs(main) do
            if inst.op == "div_k" and inst.flux == "mwi_U_p" then
                found = true
            end
        end
...
h.describe("lower: vector solve per-component expansion", function()
    ...
    h.it(
        "sys_reset, under_relax, solve_linalg emitted per component",
        function()
    ...
    h.it("emits vector BC closes for component systems", function()
        ...
        h.expect(
            has_any(main, op_field_patch("patch_s_close_d", "U_x", "north"))
        )
            .is_truthy()
        h.expect(
            has_any(main, op_field_patch("patch_s_close_d", "U_y", "north"))
        )
            .is_truthy()
        h.expect(
            has_any(main, op_field_patch("patch_s_close_d", "U_x", "south"))
        )
            .is_truthy()
        h.expect(
            has_any(main, op_field_patch("patch_s_close_d", "U_y", "south"))
        )
            .is_truthy()

        h.expect(
            has_any(main, op_field_patch("patch_s_close_n", "U_x", "east"))
        )
            .is_truthy()
        h.expect(
            has_any(main, op_field_patch("patch_s_close_n", "U_y", "east"))
        )
            .is_truthy()
    end)
...
h.describe(
    "lower: div(mwi) explicit source on RHS (pressure correction)",
    function()
        local main
        ...
        h.it("su_fs sources __mwidiv_mwi_U_p not __coeff", function()
            ...
            h.expect(false)
                .is_truthy("su_fs sourcing __mwidiv_mwi_U_p not found")
        end)
        ...
        h.it(
            "no eval_coeff emitted (divergence is not a pointwise expression)",
            function()
        ...
        h.it(
            "rhie_chow is emitted before divergence uses mwi face field",
            function()
                local rc = pos_of(main, op("rhie_chow"))
                local dv = pos_of(main, op("divergence"))
        ...
        h.it(
            "rhie_chow writes the same mwi face field consumed by divergence",
            function()
                local rc_out
                for _, inst in ipairs(main) do
...
h.describe("lower: div(U + W) explicit source", function()
    ...
    h.it(
        "does not use rhie_chow for ordinary vector expression flux",
        function()
            h.expect(has_any(main, op("rhie_chow"))).is_falsy()
        end
    )
    ...
    h.it("lap_f coefficient is the __divcell_N field (not __coeff)", function()
        for _, inst in ipairs(main) do
            if inst.op == "lap_f" then
                h.expect(inst.coeff:find("^__divcell_"))
                    .is_not_nil("lap_f coeff is '" .. tostring(inst.coeff) .. "'")
                return
            end
        end
...
h.describe("lower: scaled div(U) coefficient (nu * div(U))", function()
    ...
    h.it(
        "full ordering: divergence_c < sys_reset < eval_coeff < lap_f < under_relax",
        function()
            local dc = pos_of(main, op("divergence_c"))
            local rst = pos_of(main, op("sys_reset"))
...
h.describe("lower_equation: independent of full alg compilation", function()
    h.it(
        "lower_equation result matches full compile main for single-equation case",
        function()
...
h.describe(
    "lower: vector evaluate resolves to scalar component evals",
    function()
        local main
...
h.describe("lower: zero instruction", function()
    ...
    h.before_each(function()
        local reg, alg = make_poisson()
        alg = Alg.new("zero-poisson"):loop(function(a)
            a:zero("phi")
            a:solve("phi")
        end, 1)
...
h.describe("lower: vector config inheritance", function()
    h.it(
        "under-relaxed vector solve components inherit parent relax value",
        function()
            local reg, alg = make_momentum()
            ...
            h.expect(listing:find("UNDER_RELAX   U_x  alpha=0.7", 1, true))
                .is_not_nil()
            h.expect(listing:find("UNDER_RELAX   U_y  alpha=0.7", 1, true))
                .is_not_nil()
        end
    )
end)
