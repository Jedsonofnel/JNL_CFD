...
local function make_field(name)
	...
	function t:fill(v) self._calls[#self._calls + 1] = { "fill", v } end

	function t:clamp(lo, hi) self._calls[#self._calls + 1] = { "clamp", lo, hi } end

	function t:axpy(a, _) self._calls[#self._calls + 1] = { "axpy", a } end

	function t:copy_from(_) self._calls[#self._calls + 1] = { "copy_from" } end

	function t:norm_l2() return 0.0 end
...
local function make_sys(name)
	local t = { _name = name, _calls = {} }
	function t:reset() self._calls[#self._calls + 1] = { "reset" } end

	function t:under_relax(_, a) self._calls[#self._calls + 1] = { "under_relax", a } end
...
local function make_case(fields, systems, cfg_overrides, exec_overrides)
	local field_map = {}
	local sys_map   = {}
	for k, v in pairs(fields or {}) do field_map[k] = v end
	for k, v in pairs(systems or {}) do sys_map[k] = v end
	return {
		field_map = field_map,
		sys_map   = sys_map,
		...
		bindings  = {},
		compiled  = {
			reg = { entry = function() return nil end }
		},
	}
end
...
local function capture_B(name, fn)
	local calls = {}
	with_mock_B(name, function(...) calls[#calls + 1] = { ... } end, fn)
	return calls
end
...
h.describe("dispatch: comment", function()
	h.it("is a no-op, does not error", function()
		h.expect(function() D.comment(nil, { op = "comment", text = "hi" }) end)
			.not_throws()
	end)
end)
...
h.describe("dispatch: div_k scheme routing", function()
	local function test_scheme(scheme)
		...
		local case = make_case({ ["mwi_U_p"] = un }, { phi = s }, { div = scheme })
		local got  = {}

		with_mock_B("div_uds_k", function() got[#got + 1] = "div_uds_k" end, function()
			with_mock_B("div_cds_k", function() got[#got + 1] = "div_cds_k" end, function()
...
h.describe("dispatch: div_dc no-op for non-tvd schemes", function()
	local function run_div_dc(scheme)
		...
		local calls = {}
		local function mock_tvd() calls[#calls + 1] = "tvd" end
...
h.describe("dispatch: div_dc tvd limiter routing", function()
	local function limiter_fn(limiter)
		...
		local called = {}
		with_mock_B("div_tvd_minmod", function() called[#called + 1] = "minmod" end, function()
			with_mock_B("div_tvd_van_leer", function() called[#called + 1] = "van_leer" end, function()
				with_mock_B("div_tvd_superbee", function() called[#called + 1] = "superbee" end, function()
					D.div_dc(case, {
						field  = "phi",
						flux   = "mwi_U_p",
						grad_x = "grad_phi_x",
...
h.describe("dispatch: su_k volumetric vs integrated routing", function()
	local function run_su_k(volumetric)
		...
		local called = {}
		with_mock_B("su_v_k", function() called[#called + 1] = "v_k" end, function()
			with_mock_B("su_i_k", function() called[#called + 1] = "i_k" end, function()
				D.su_k(case, { field = "phi", coeff = 1.0, volumetric = volumetric })
			end)
		end)
		return called[1]
	end
