...
local function run_pressure_correction(c, mesh, scale)
    ...
    for _ = 1, 2000 do
        ...
        if st.done or st.breakdown then
            break
        end
    end
...
h.describe("pressure correction: null test (exact Couette input)", function()
    h.it(
        "divergence is near-zero before correction on exact profile",
        function()
            local div_before, _ = correction_run(0.0, -1.0)
            h.expect(div_before)
                .is_less_than(1e-8, "div_before on exact profile: " .. tostring(div_before))
        end
    )
    ...
    h.it(
        "divergence after correction is not larger than before on exact profile",
        function()
            local div_before, div_after = correction_run(0.0, -1.0)
            h.expect(div_after).is_less_than(
                div_before + 1e-12,
                string.format(
                    "div grew on exact profile: before=%.4e after=%.4e",
                    div_before,
                    div_after
                )
            )
...
h.describe("pressure correction: source sign diagnostic", function()
    ...
    h.it("scale=+1 amplifies divergence (confirms sign convention)", function()
        local div_before, div_after = correction_run(0.05, 1.0)
        h.expect(div_after).is_greater_than(
            div_before,
            string.format(
...
h.describe("pressure correction: diagonal scaling", function()
    ...
    h.it(
        "correction still reduces divergence for 10x stiffer diagonal",
        function()
            local div_before, div_after =
                correction_run(0.05, -1.0, 4.0 * NU * 10.0)
            h.expect(div_after).is_less_than(
                div_before,
                string.format(
    ...
    h.it(
        "correction still reduces divergence for 10x softer diagonal",
        function()
            local div_before, div_after =
                correction_run(0.05, -1.0, 4.0 * NU * 0.1)
            h.expect(div_after).is_less_than(
                div_before,
                string.format(
