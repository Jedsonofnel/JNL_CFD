...
local function list_dir(path)
    local cmd = is_windows and string.format('dir /b /a "%s" 2>nul', path)
...
local function infer_lua_root()
    local source = (debug.getinfo(1, "S").source or ""):match("^@(.+)$") or ""

    for entry in package.path:gmatch("[^;]+") do
        local root = entry:match("^(.-)%?")

        if root and root ~= "" and source:sub(1, #root) == root then
            return root:gsub("[/\\]$", "")
        end
    end
...
function M.describe(name, fn)
    ...
    assert(type(fn) == "function", "h.describe: callback must be a function")
...
function M.it(name, fn)
    ...
    assert(type(fn) == "function", "h.it: callback must be a function")
...
function M.before_each(fn)
    assert(type(fn) == "function", "h.before_each: callback must be a function")

    local hooks = current and current.before_each or suite_before_each
...
function M.after_each(fn)
    assert(type(fn) == "function", "h.after_each: callback must be a function")

    local hooks = current and current.after_each or suite_after_each
...
function M.expect(value)
    ...
    function expectation.equals(expected)
        if value ~= expected then
            fail(
                string.format("expected %s, got %s", fmt(expected), fmt(value))
            )
        end
    ...
    function expectation.is_nil(message)
        if value ~= nil then
            fail(message or string.format("expected nil, got %s", fmt(value)))
        end
    ...
    function expectation.is_truthy(message)
        if not value then
            fail(
                message or string.format("expected truthy, got %s", fmt(value))
            )
        end
    ...
    function expectation.is_falsy(message)
        if value then
            fail(message or string.format("expected falsy, got %s", fmt(value)))
        end
    ...
    function expectation.is_less_than(number, message)
        if not (type(value) == "number" and value < number) then
            fail(
                message
                    or string.format(
                        "expected < %s, got %s",
...
local function run_hook(hook, label, index)
    ...
    if not ok then
        fail(string.format("%s hook %d: %s", label, index, tostring(err)))
    end
...
local function run_test(block, test)
    ...
    local setup_ok = run_before_hooks(suite_before_each, "suite before_each")

    if setup_ok then
        setup_ok = run_before_hooks(block.before_each, "before_each")
    end
    ...
    run_after_hooks(block.after_each, "after_each")

    run_after_hooks(suite_after_each, "suite after_each")
...
function M.run_specs(base_dir, lua_root)
    ...
    for _, path in ipairs(files) do
        local module_name = path_to_module(path, lua_root)
