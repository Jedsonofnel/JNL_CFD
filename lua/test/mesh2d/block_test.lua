...
local function sq_south()
    return curve.line(0, 0, 1, 0)
end
local function sq_east()
    return curve.line(1, 0, 1, 1)
end
local function sq_north()
    return curve.line(0, 1, 1, 1)
end
local function sq_west()
    return curve.line(0, 0, 0, 1)
end
...
h.describe("BlockBuilder: single block", function()
    ...
    h.it("smooth after tfi produces a mesh", function()
        ...
        local mesh, err = blk.block(9, 9)
            :south(sq_south(), { marker = WALL })
            :east(
                curve.arc(1, 0.5, 0.5, -math.pi / 2, math.pi / 2),
                { marker = OUTLET }
            )
            :north(sq_north(), { marker = TOP })
            :west(sq_west(), { marker = INLET })
...
h.describe("BlockBuilder: from_pen", function()
    ...
    local function cw_unit_pen()
        return pen
            ...
            :at(0, 0)
            :east(1)
            :tag("s") -- south: (0,0)->(1,0)  correct orientation
            :north(1)
            :tag("e") -- east:  (1,0)->(1,1)  correct orientation
            :west(1)
            :tag("n") -- north: (1,1)->(0,1)  reversed
            :close()
            :tag("w") -- west:  (0,1)->(0,0)  reversed
    end
...
h.describe("GridBuilder: edge validation", function()
    h.it("assigned join-target edge returns error", function()
        ...
        b1
            :south(curve.line(1, 0, 2, 0), { marker = WALL })
