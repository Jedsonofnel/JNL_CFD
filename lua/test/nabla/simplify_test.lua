...
h.describe("simplify: anonymous constant folding", function()
    h.it("multi-term sum folds anon constants", function()
        ...
        if s.kind == "add" then
            local b = s.b or s.a
            if b.kind == "constant" and b.a == 5 then
                found_five = true
            end
        end
    ...
    h.it("anon constants fold in mul", function()
        ...
        if s.kind == "mul" then
            if
                (s.a.kind == "constant" and s.a.a == 6)
                or (s.b and s.b.kind == "constant" and s.b.a == 6)
            then
                found_six = true
            end
        end
...
h.describe("simplify: named constants never folded", function()
    h.it("named constant preserved in sum with anon", function()
        ...
        local s = simplify(n)
        h.expect(s.kind)
            .not_equals("constant", "named const folded with anon — should be preserved")
    end)
    ...
    h.it("named constant factor not treated as coefficient", function()
        ...
        local s = simplify(n)
        h.expect(s)
            .not_equals(p, "named const rho=1 collapsed — should be preserved")
    end)
