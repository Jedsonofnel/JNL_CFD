...
h.describe("REPL registration", function()
    h.it("exposes a value as a global", function()
        ...
        instance:register("sample-value", value, "A sample value.")
    ...
    h.it("stores the value in the help registry", function()
        ...
        instance:register("sample-entry", value, "A sample entry.")
    ...
    h.it("returns the registered value", function()
        ...
        local returned = instance:register("returned-value", value, false)
    ...
    h.it("accepts literal documentation", function()
        ...
        instance:register("literal-doc", function() end, "Literal help text.")

        h.expect(instance.registry["literal-doc"].doc)
    ...
    h.it("suppresses documentation lookup with false", function()
        ...
        instance:register("default_scheme", function() end, false)

        h.expect(instance.registry.default_scheme.doc).equals("")
    end)
    ...
    h.it("looks up documentation by registered name", function()
        ...
        instance:register("default_scheme", function() end)

        h.expect(instance.registry.default_scheme.doc)
    ...
    h.it("looks up documentation from an explicit symbol", function()
        ...
        instance:register("scheme", function() end, {
            from = "test.doc_sample.default_scheme",
        })

        h.expect(instance.registry.scheme.doc)
            .equals("Return the default scheme.")
    end)
    ...
    h.it("allows literal documentation in a table", function()
        ...
        instance:register("table-doc", 42, {
            doc = "Documentation from a specification table.",
        })

        h.expect(instance.registry["table-doc"].doc)
            .equals("Documentation from a specification table.")
    end)
...
h.describe("Default REPL facade", function()
    ...
    h.it("module registration uses the default instance", function()
        ...
        local entry = repl.default().registry["default-instance-value"]
    ...
    h.it("module special stores a global", function()
        ...
        local value = repl.special("*sample-result*", 123, false)
...
h.describe("REPL usage", function()
    h.it("returns literal usage text", function()
        ...
        h.expect(instance:usage_string()).equals("Run `(demo)`.")
    end)
    ...
    h.it("calls a usage provider", function()
        ...
        h.expect(instance:usage_string()).equals("Generated usage.")
    end)
    ...
    h.it("accepts an object with string method", function()
        ...
        h.expect(instance:usage_string()).equals("Object usage.")
    end)
    ...
    h.it("accepts an object with usage_string method", function()
        ...
        h.expect(instance:usage_string()).equals("Provider usage.")
    end)
    ...
    h.it("returns fallback usage when none is registered", function()
        ...
        h.expect(
            text:find("No study-specific usage has been registered.", 1, true)
        )
            .is_not_nil()
    end)
end)
...
h.describe("REPL commands", function()
    h.it("registers a custom command", function()
        ...
        instance:command("sample", function(active, argument)
            h.expect(active).equals(instance)
            h.expect(argument).equals("hello")
            called = true
        end, ",sample <text>", "Run the sample command.")

        instance.commands.sample.fn(instance, "hello")

        h.expect(called).is_truthy()
        h.expect(instance.commands.sample.usage).equals(",sample <text>")
    end)
