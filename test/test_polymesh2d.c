...
static void assert_mesh_ok(struct jnl_polymesh2d *m)
{
	...
	TEST_ASSERT_MSG(err == JNL_MESH_OK, "mesh check failed: %s",
...
static void test_one_quad_boundary(void)
{
	...
	TEST_ASSERT(d->cell_marker && d->cell_vertex_start && d->cell_vertex_list);
	...
	TEST_ASSERT(m->topo.n_vertices == 4);
	TEST_ASSERT(m->topo.n_real_cells == 1);
	TEST_ASSERT(m->topo.n_ghost_cells == 4);
	TEST_ASSERT(m->topo.n_cells == 5);

	TEST_ASSERT(m->topo.n_internal_faces == 0);
	TEST_ASSERT(m->topo.n_boundary_faces == 4);
	TEST_ASSERT(m->topo.n_baffle_faces == 0);
	TEST_ASSERT(m->topo.n_faces == 4);

	TEST_ASSERT(m->patches.n_patches == 4);
	TEST_ASSERT(m->regions.n_regions == 1);
	TEST_ASSERT(m->baffles.n_baffles == 0);
	...
	for (i32 f = 0; f < m->topo.n_faces; f++) {
		...
		TEST_ASSERT(o == 0);
		TEST_ASSERT(n >= m->topo.n_real_cells);
		TEST_ASSERT(m->topo.cell_kind[n] == JNL_PMSH2D_CELL_GHOST);
		TEST_ASSERT(m->topo.face_kind[f] == JNL_PMSH2D_FACE_BOUNDARY);
		TEST_ASSERT(m->topo.face_patch[f] >= 0);
		TEST_ASSERT(m->topo.face_baffle[f] == -1);
		TEST_ASSERT(m->topo.paired_face[f] == -1);
	...
	for (i32 p = 0; p < m->patches.n_patches; p++)
		TEST_ASSERT(m->patches.data[p].n_faces == 1);

	TEST_ASSERT(m->regions.data[0].start_cell == 0);
	TEST_ASSERT(m->regions.data[0].n_cells == 1);
...
static void test_two_quads_internal_face(void)
{
	...
	TEST_ASSERT(m->topo.n_cells == 8);

	TEST_ASSERT(m->topo.n_internal_faces == 1);
	TEST_ASSERT(m->topo.n_boundary_faces == 6);
	TEST_ASSERT(m->topo.n_baffle_faces == 0);
	TEST_ASSERT(m->topo.n_faces == 7);

	i32 f = 0;
	TEST_ASSERT(m->topo.face_kind[f] == JNL_PMSH2D_FACE_INTERNAL);
	TEST_ASSERT(m->topo.owner[f] >= 0);
	TEST_ASSERT(m->topo.owner[f] < m->topo.n_real_cells);
	TEST_ASSERT(m->topo.neighbour[f] >= 0);
	TEST_ASSERT(m->topo.neighbour[f] < m->topo.n_real_cells);
	TEST_ASSERT(m->topo.face_patch[f] == -1);
	TEST_ASSERT(m->topo.face_baffle[f] == -1);
	TEST_ASSERT(m->topo.paired_face[f] == -1);
	...
	TEST_ASSERT(m->regions.data[0].start_cell == 0);
	TEST_ASSERT(m->regions.data[0].n_cells == 2);
...
static void test_two_quads_baffle_face_pair(void)
{
	...
	TEST_ASSERT(m->topo.n_real_cells == 2);
	TEST_ASSERT(m->topo.n_ghost_cells == 8);
	TEST_ASSERT(m->topo.n_cells == 10);

	TEST_ASSERT(m->topo.n_internal_faces == 0);
	TEST_ASSERT(m->topo.n_boundary_faces == 6);
	TEST_ASSERT(m->topo.n_baffle_faces == 2);
	TEST_ASSERT(m->topo.n_faces == 8);

	TEST_ASSERT(m->baffles.n_baffles == 1);
	TEST_ASSERT(m->baffles.n_baffle_faces == 2);
	TEST_ASSERT(m->baffles.data[0].n_faces == 2);
	TEST_ASSERT(m->baffles.data[0].n_pairs == 1);

	i32 f0 = m->baffles.data[0].face0[0];
	i32 f1 = m->baffles.data[0].face1[0];

	TEST_ASSERT(f0 >= m->baffles.data[0].start_face);
	TEST_ASSERT(f1 >= m->baffles.data[0].start_face);
	TEST_ASSERT(f0 != f1);

	TEST_ASSERT(m->topo.paired_face[f0] == f1);
	TEST_ASSERT(m->topo.paired_face[f1] == f0);

	TEST_ASSERT(m->topo.face_kind[f0] == JNL_PMSH2D_FACE_BAFFLE);
	TEST_ASSERT(m->topo.face_kind[f1] == JNL_PMSH2D_FACE_BAFFLE);

	TEST_ASSERT(m->topo.face_baffle[f0] == 0);
	TEST_ASSERT(m->topo.face_baffle[f1] == 0);

	TEST_ASSERT(m->topo.cell_kind[m->topo.neighbour[f0]] ==
	            JNL_PMSH2D_CELL_GHOST);
	TEST_ASSERT(m->topo.cell_kind[m->topo.neighbour[f1]] ==
	            JNL_PMSH2D_CELL_GHOST);
...
int main(void)
{
	test_one_quad_boundary();
	test_two_quads_internal_face();
	test_two_quads_baffle_face_pair();
	test_unknown_patch_marker_fails();
